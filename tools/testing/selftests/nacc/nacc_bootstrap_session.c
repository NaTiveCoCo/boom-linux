...
#define TEST_RX_BASE UINT64_C(0x0000000100000000)
#define TEST_RX_SIZE UINT64_C(0x2000)
#define TEST_RUNTIME_ENTRY (TEST_RX_BASE + UINT64_C(0x100))
...
static int arm(struct nacc_linux_bootstrap_session *session)
{
	return nacc_linux_bootstrap_session_arm(
		...
		TEST_OLD_SATP, TEST_CONTROL_SATP, TEST_RX_BASE, TEST_RX_SIZE);
}
...
static struct nacc_linux_bootstrap_ready_request valid_ready(void)
{
	return (struct nacc_linux_bootstrap_ready_request) {
		...
		.runtime_entry_address = TEST_RUNTIME_ENTRY,
	};
}
...
static void test_arm_validation(void)
{
	...
	report_contract(nacc_linux_bootstrap_session_arm(
			...
			TEST_RX_BASE, TEST_RX_SIZE) == -EINVAL,
	...
	report_contract(nacc_linux_bootstrap_session_arm(
			...
			TEST_CONTROL_SATP, TEST_RX_BASE, TEST_RX_SIZE) == -EINVAL,
	...
	report_contract(nacc_linux_bootstrap_session_arm(
			&session, TEST_CONTEXT, TEST_SEQUENCE, TEST_TP,
			0, TEST_CONTROL_SATP, TEST_RX_BASE, TEST_RX_SIZE) == -EINVAL,
	...
	report_contract(nacc_linux_bootstrap_session_arm(
			&session, TEST_CONTEXT, TEST_SEQUENCE, TEST_TP,
			TEST_OLD_SATP, TEST_OLD_SATP,
			TEST_RX_BASE, TEST_RX_SIZE) == -EINVAL,
			 "identical old and control roots are rejected");
	memset(&session, 0, sizeof(session));
	report_contract(nacc_linux_bootstrap_session_arm(
			&session, TEST_CONTEXT, TEST_SEQUENCE, TEST_TP,
			TEST_OLD_SATP, TEST_OLD_SATP ^ (1ULL << 44),
			TEST_RX_BASE, TEST_RX_SIZE) == -EINVAL,
			 "one root PPN with different ASIDs is rejected");
	memset(&session, 0, sizeof(session));
	report_contract(nacc_linux_bootstrap_session_arm(
			&session, TEST_CONTEXT, TEST_SEQUENCE, TEST_TP,
			TEST_OLD_SATP, TEST_CONTROL_SATP, 0,
			TEST_RX_SIZE) == -EINVAL,
			 "zero Agent RX base is rejected");
	memset(&session, 0, sizeof(session));
	report_contract(nacc_linux_bootstrap_session_arm(
			&session, TEST_CONTEXT, TEST_SEQUENCE, TEST_TP,
			TEST_OLD_SATP, TEST_CONTROL_SATP, TEST_RX_BASE + 1,
			TEST_RX_SIZE) == -EINVAL,
			 "misaligned Agent RX base is rejected");
	memset(&session, 0, sizeof(session));
	report_contract(nacc_linux_bootstrap_session_arm(
			&session, TEST_CONTEXT, TEST_SEQUENCE, TEST_TP,
			TEST_OLD_SATP, TEST_CONTROL_SATP, TEST_RX_BASE,
			NACC_LINUX_RUNTIME_ENTRY_INSN_SIZE - 1) == -EINVAL,
			 "short Agent RX range is rejected");
	memset(&session, 0, sizeof(session));
	report_contract(nacc_linux_bootstrap_session_arm(
			&session, TEST_CONTEXT, TEST_SEQUENCE, TEST_TP,
			TEST_OLD_SATP, TEST_CONTROL_SATP,
			NACC_BOOTSTRAP_SV39_USER_LIMIT - NACC_BOOTSTRAP_PAGE_SIZE,
			NACC_BOOTSTRAP_PAGE_SIZE + 1) == -EINVAL,
			 "Agent RX range outside the user half is rejected");
}
...
static void test_ready_validation(void)
{
	...
	changed = request;
	changed.runtime_entry_address =
		TEST_RX_BASE + TEST_RX_SIZE - NACC_LINUX_RUNTIME_ENTRY_INSN_SIZE;
	report_contract(!nacc_linux_bootstrap_session_ready_validate(
			&session, &changed),
			 "last complete instruction in Agent RX validates");
...
#define REJECT_CHANGED(_field, _value, _name) do { \
	...
	REJECT_CHANGED(runtime_entry_address, 0,
		       "READY zero runtime entry is rejected");
	REJECT_CHANGED(runtime_entry_address, TEST_RUNTIME_ENTRY + 2,
		       "READY misaligned runtime entry is rejected");
	REJECT_CHANGED(runtime_entry_address, TEST_RX_BASE - 4,
		       "READY runtime entry below Agent RX is rejected");
	REJECT_CHANGED(runtime_entry_address, TEST_RX_BASE + TEST_RX_SIZE,
		       "READY runtime entry at Agent RX end is rejected");
	changed = request;
	changed.reserved[0] = 1;
	report_contract(nacc_linux_bootstrap_session_ready_validate(
			&session, &changed) == -EINVAL,
			 "READY a5 reserved argument is rejected");
	changed = request;
	changed.reserved[1] = 1;
	report_contract(nacc_linux_bootstrap_session_ready_validate(
			&session, &changed) == -EINVAL,
			 "READY a6 reserved argument is rejected");
	...
	report_contract(!ret &&
			...
			session.runtime_entry_address == TEST_RUNTIME_ENTRY,
			 "valid READY commits cookie and runtime entry");
...
int main(void)
{
	...
	report_contract(sizeof(struct nacc_linux_bootstrap_ready_request) == 64,
			 "READY register snapshot remains 64 bytes");
	test_arm_validation();
